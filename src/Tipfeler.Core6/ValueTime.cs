...
/// Time value.
/// </summary>
public sealed class ValueTime : Value {

    /// <summary>
    /// Create a new instance.
    /// </summary>
    /// <param name="value">Value.</param>
    public ValueTime(TimeOnly value) {
        _value = value;
    }
    ...
    public static ValueTime Parse(string text) {
    ...
    public static bool TryParse(string? text, [NotNullWhen(true)] out ValueTime? result) {
        if (TryParseValue(text, out var value)) {
            result = new ValueTime(value);
            return true;
        } else {
            result = default;
            return false;
    ...
    internal static bool TryParseValue(string? text, out TimeOnly result) {
        if (DateTime.TryParseExact(text, ValueDateTime.ParseTimeFormats, CultureInfo.InvariantCulture, ValueDateTime.ParseStyle, out var resultTime)) {
    ...
    public override string ToString() {
        return ToString(ValueDateTime.ParseTimeFormats[0]);
    }
    ...
    public static implicit operator TimeOnly(ValueTime obj)
        => obj.Value;
    ...
    /// </summary>
    /// <param name="obj">Value object.</param>
    public static implicit operator string(ValueTime obj)
        => obj.ToString();
