...
/// Date value.
/// </summary>
public sealed class TiniValueDate : TiniValue {

    /// <summary>
    /// Create a new instance.
    /// </summary>
    /// <param name="value">Value.</param>
    public TiniValueDate(DateOnly value) {
        _value = value;
    }
    ...
    public static TiniValueDate Parse(string text) {
    ...
    public static bool TryParse(string? text, [NotNullWhen(true)] out TiniValueDate? result) {
        if (TryParseValue(text, out var value)) {
            result = new TiniValueDate(value);
            return true;
        } else {
            result = default;
            return false;
    ...
    internal static bool TryParseValue(string? text, out DateOnly result) {
        if (DateTime.TryParseExact(text, TiniValueDateTime.ParseDateFormats, CultureInfo.InvariantCulture, TiniValueDateTime.ParseStyle, out var resultDate)) {
    ...
    public override string ToString() {
        return ToString(TiniValueDateTime.ParseDateFormats[0]);
    }
    ...
    public static implicit operator DateOnly(TiniValueDate obj)
        => obj.Value;
    ...
    /// </summary>
    /// <param name="obj">Value object.</param>
    public static implicit operator string(TiniValueDate obj)
        => obj.ToString();
