...
internal partial class MainForm : Form {
    ...
    private void mnuOpen_Click(object sender, EventArgs e) {
        if (ProceedWithNewDocument()) {
            using (var frm = new OpenFileDialog() { Filter = "AASeq documents (*.aaseq)|*.aaseq|All files (*.*)|*.*" }) {
    ...
    private void mnuSaveAs_Click(object sender, EventArgs e) {
        using (var frm = new SaveFileDialog() { AddExtension = true, DefaultExt = "aaseq", Filter = "AASeq documents (*.aaseq)|*.aaseq|All files (*.*)|*.*" }) {
            if (frm.ShowDialog(this) == DialogResult.OK) {
                try {
    ...
    private void mnuAppFeedback_Click(object sender, EventArgs e) {
        Medo.Diagnostics.ErrorReport.ShowDialog(this, null, new Uri("https://api.aaseq.com/feedback/"));
    }

    private void mnuAppUpdate_Click(object sender, EventArgs e) {
        Medo.Services.Upgrade.ShowDialog(this, new Uri("https://api.aaseq.com/upgrade/"));
    }

    private void mnuAppAbout_Click(object sender, EventArgs e) {
        Medo.Windows.Forms.AboutBox.ShowDialog(this, new Uri("https://aaseq.com/"));
    }
    ...
    private void RefreshTitle() {
        if (DocumentFileName != null) {
            Text = Path.GetFileNameWithoutExtension(DocumentFileName) + (Document.IsChanged ? "*" : "") + " - AASeq";
        } else {
            Text = "AASeq" + (Document.IsChanged ? "*" : "");
        }
    }
