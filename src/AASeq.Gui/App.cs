...
static class App {
    ...
    private static readonly Mutex SetupMutex = new Mutex(false, @"Global\AASeq");
...
#if !DEBUG
            Medo.Diagnostics.ErrorReport.ShowDialog(null, e.Exception, new Uri("https://api.aaseq.com/feedback/"), string.Format("{0} at {1}\\{2}", Environment.UserName, Environment.UserDomainName, Environment.MachineName));
#else
        throw e.Exception;
#endif
    }
